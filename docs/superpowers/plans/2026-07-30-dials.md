...
def _keysym_name(display, keycode: int) -> str:
    ...
    `XK.keysym_to_string` does NOT simply return None for keysyms it cannot
    render - it carries an explicit special-case list (BackSpace, Tab, Clear,
    Return, Pause, Escape, Delete and Scroll_Lock) for which it returns
    `chr(keysym & 0xff)`. So `Scroll_Lock` (0xFF14) yields '\\x14', which is
    truthy and would shadow the table. Ordering it first would silently drop
    ScrollLock's modifier from the tolerated set, and every Dial would die
    whenever ScrollLock was on - the same silent failure this whole module
    exists to prevent, one lock key over.

    The `if not ks` guard is load-bearing for the same reason: NoSymbol is
    keysym 0, and `keysym_to_string(0)` returns a truthy '\\x00', so an
    unmapped keycode would otherwise resolve to '\\x00' rather than "".
    Keysym 0 is the protocol's "nothing bound here" sentinel and no real lock
    key uses it, so the guard cannot swallow a legitimate key.
    """
    from Xlib import XK
    try:
        ks = display.keycode_to_keysym(keycode, 0)
        if not ks:                      # NoSymbol - see docstring
            return ""
