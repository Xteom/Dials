    ...
    def monitors(self) -> list[Monitor]:
        if self._cache is None:
            try:
                ...
                # Degrade, never crash: keep whatever we had. On a cold cache
                # this yields [], and pick() falls back to the root box.
...
def test_daemon_never_imports_the_writer():
    """The writer pulls in tomli_w; the daemon's budget says it must not."""
    import pathlib
    src = pathlib.Path("dials/daemon.py")
    if src.exists():
        assert "configwrite" not in src.read_text()
```
...
def test_panel_hints_are_the_four_from_the_spec():
    assert PANEL_HINTS == (
