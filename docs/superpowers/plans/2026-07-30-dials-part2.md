...
def test_select_timeout_picks_the_nearest_deadline():
    ...
    d.handle_slot("9", timestamp=1)      # confirm deadline at +5
    d.assign.arm.__self__  # assign exists
    from dials.assign import Capture
    d.assign.arm(Capture(1, "x", "x", "HDMI-0", (0, 0, 1, 1), clk.t))  # +5 too
    clk.advance(4.5)
    t = d.select_timeout()
    assert 0 <= t <= 0.6
...
def main() -> int:
    ...
    while True:
        ...
        if need and not confirm_grabs_held:
            ...
            if len(failed) == len(daemon.launcher.confirm_keycodes()):
                ...
                _confirm_grabs = extra
        elif confirm_grabs_held and not need:
            _confirm_grabs.remove_all()
            confirm_grabs_held = False
...
def test_cell_label_never_exceeds_the_cell_width():
    long = dial()
    object.__setattr__(long, "label", "An Extremely Long Application Name")
