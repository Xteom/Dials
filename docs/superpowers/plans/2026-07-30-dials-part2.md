...
def _run_dialog(data: dict) -> int:
    import gi
    # BOTH namespaces must be pinned. This machine has Gdk-3.0 AND Gdk-4.0
    # installed (normal on modern Pop!_OS), so an unpinned `from gi.repository
    # import Gdk` resolves to 4.0 and the subsequent Gtk 3.0 load then dies with
    # "Requiring namespace 'Gdk' version '3.0', but '4.0' is already loaded".
    # main()'s except-branch would swallow that and merely refuse the overwrite,
    # so the dialog would NEVER appear and the user would see a plausible
    # refusal instead of an error - the safety fallback masking total breakage.
...
def main(argv=None) -> int:
    ...
    if status == 0:
        ...
        dial = Dial(
            ...
            # MUST be None, never the existing Dial's launch. The only producer
            # of an OverwriteRequest is AssignMode.resolve, which always builds
            # `incoming` with launch=None because a freshly captured window has
            # no launch command. Inheriting the destroyed Dial's launch would
            # attach it to the NEW match_class - replacing a Spotify Dial with
            # Slack would keep launching Spotify.
            launch=None,
            icon="",
